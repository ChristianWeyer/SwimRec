...
function refreshFavoritesRecordsCore(){var a=[];store.each(function(b){var c=recordsUrl.replace("{0}",b.localSwimmer.Swimmer.SwimmerID);a.push($.ajax({url:c,cache:!1,type:"GET",processData:!1,error:function(a){hideLoader();alert(a)},success:function(a){b.localSwimmer.Records=a;store.save({key:b.key,localSwimmer:b.localSwimmer},function(){})}}))});return a}
function searchSwimmers(){showLoader("Suche...");var a=$("#search").val().split(" "),b=a[0],c=a[1],a=swimmersUrl.replace("{0}",b).replace("{1}",c);$.ajax({url:a,cache:!1,type:"GET",dataType:"json",error:function(a,b){hideLoader();showDialog("serviceErrorDialog");console.log(JSON.stringify(a));console.log(JSON.stringify(b))},success:function(a){console.log(JSON.stringify(a));foundSwimmers=a;currentSwimmerName=b+" "+c;null==foundSwimmers||""==foundSwimmers?(showDialog("swimmerErrorDialog"),hideLoader()):
1<foundSwimmers.length?(a=kendo.template($("#swimmersTemplate").text()),$("#searchResultList").html(a(foundSwimmers)),kendo.mobile.init($("#searchResultList")),hideLoader(),window.kendoMobileApplication.navigate("#swimmerResultsPage")):searchRecords(foundSwimmers[0].SwimmerID,!1,!0)}})}
function searchRecords(a,b,c){b&&showLoader("Suche...");b=recordsUrl.replace("{0}",a);$.ajax({url:b,cache:!1,type:"GET",processData:!1,error:function(){hideLoader();showDialog("serviceErrorDialog")},success:function(b){currentSwimmer=_.find(foundSwimmers,function(b){return b.SwimmerID==a});currentSwimmerRecords=b;c&&(window.kendoMobileApplication.navigate("#recordsResultsPage"),bindSwimmer(currentSwimmerRecords),bindRecordsTables(currentSwimmerRecords));hideLoader()}})}
function bindSwimmer(){$("#currentSwimmerRecords").text(currentSwimmerName)}function bindRecordsTables(a){var b=a[0].Times,c=a[1].Times,d=[],e=0;b.forEach(function(a){a<c[e]&&""!=a||""==c[e]?d.push(a):d.push(c[e]);e++});a=kendo.template($("#resultsTemplate").text());$("#recordsList25m").html(a(b));$("#recordsList50m").html(a(c));$("#recordsListTotal").html(a(d));kendo.mobile.init($("#recordsList25m"));kendo.mobile.init($("#recordsList50m"));kendo.mobile.init($("#recordsListTotal"))};